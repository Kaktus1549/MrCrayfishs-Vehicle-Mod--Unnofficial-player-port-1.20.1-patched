    ...
    public void render(GasPumpBlockEntity entity, float delta, @NotNull PoseStack matrices, @NotNull MultiBufferSource buffers, int light, int overlay)
    {
       ...
       /* List<VehicleEntity> vehicles = te.getWorld().getEntitiesWithinAABB(VehicleEntity.class, new AxisAlignedBB(te.getPos()).grow(5.0));
        ...
        if(entity.getFuelingEntity() == null)
        {
            ...
            RenderObjectHelper.renderColoredModel(VehicleModels.NOZZLE.getBaseModel(), ItemDisplayContext.NONE, false, matrices, buffers, -1, OverlayTexture.NO_OVERLAY, light);
            matrices.popPose();
        }
        ...
        matrices.pushPose();
        {
            ...
            matrices.pushPose();
            {
                ...
                if(entity.getTank() != null)
                {
                    ...
                    font.drawInBatch(
                            percent, -width / 2, 10, DyeColor.WHITE.getTextColor(), false, matrices.last().pose(), buffers, Font.DisplayMode.NORMAL, 0, light
                    );
            ...
            matrices.pushPose();
            {
                ...
                RenderSystem.depthMask(false);
                font.drawInBatch(
                        "Fuelium", -width / 2, 10, DyeColor.LIME.getTextColor(), false, matrices.last().pose(), buffers, Font.DisplayMode.NORMAL, 0, light
                );
                RenderSystem.depthMask(true);
            }
            matrices.popPose();
