...
public class CameraHelper
{
    ...
    public void load(VehicleEntity vehicle)
    {
        ...
        this.pitchOffset = 0F;
        this.yawOffset = 0F;
        this.currentRotation = new Quaternionf().rotationXYZ(
                (float) Math.toRadians(vehicle.getViewPitch(1F)),
                (float) Math.toRadians(-vehicle.getViewYaw(1F)),
                (float) Math.toRadians(vehicle.getViewRoll(1F))
        );
